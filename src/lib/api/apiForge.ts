...
const createAPIPathProxy = <T>(config: HttpConfig, method: string, path: string): T =>
  new Proxy(Function, {
    get(_, prop: string) {
...
export const createAPI = <T>(config: Partial<HttpConfig>): T => {
  return new Proxy(Object.create(null), {
    get: (_, method: string) => {
      if (method === "config") return config
      return createAPIPathProxy({ ...defaultConfig, ...config }, method.toUpperCase(), "")
    }
  })
}
